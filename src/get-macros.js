...
function getMacros(document,config){
    ...
    for (let index = 0; index < document.lineCount; index++) {
        ...
        while (true) {
            let matchMacro = lineText.match(/(?<init>\\(?:new|renew|provide)command)\s*(?<star>\*?)\s*(?<cmd>(?:\\\w+|\{[\\\w]+\})(?:\s*\[\d+\]\s*){0,2})(\\\S+|\{(?<carry>.*)\})/)
            if(!matchMacro) break;
            if(!!matchMacro.groups.carry && matchMacro.groups.carry.indexOf("}")!== -1){
                let carry = matchMacro.groups.carry
                let init = matchMacro.groups.init+matchMacro.groups.cmd
                let depth = 1
            ...
            if(!!matchMacro.groups.star) matchMacro[0]=matchMacro[0].replace(matchMacro.groups.star, "")
